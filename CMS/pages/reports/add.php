...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$emptyFields) {
        ...
        $destinataires=loadusers();
        $destinataireToSendTo;
        foreach($destinataires as $destinataire) {
            if($destinataire->getEmail() == $_POST['email']) {
                $destinataireToSendTo = $destinataire;
                break;
            }
        }
        $rapportToSave= new Rapport(1000 ,
            $date,
            $_POST['contenu'],
            $destinataireToSendTo,
            $destinataireToSendTo);
        $saved = saveRapport($rapportToSave);
        if($saved){
            echo "<script>alert('Rapport envoyé avec succes');</script>";
        }
        else{
            // If update fails
            echo "<script>alert('rapport invalide');</script>";
        }
    } else {
        // If info are wrong
...
<?php include_once "../components/navbar.php";?>
      <div class="container-fluid m-0 p-0 ">
        ...
        <form class="p-3 " method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <div class="row mb-3">
                <div class="col-12">
                    <label for="email" class="mb-3">Destinataire ( email ) </label>
            ...
            <button type="submit" class=" btn btn-warnings  fw-bold">Enregistrer rapport</button>
         </form>
        </main>
        </div>
