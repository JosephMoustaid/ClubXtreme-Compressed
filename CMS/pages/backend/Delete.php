...
function deleteMember(int $id) {
    global $db;
    try {
        $query1 = $db->prepare("DELETE FROM transaction WHERE id_membre = :id");
        $query1->bindValue(':id', $id);
        $query1->execute();
        $query2 = $db->prepare("DELETE FROM participationevenements WHERE id_participant = :id");
        $query2->bindValue(':id', $id);
        $query2->execute();
        $query3 = $db->prepare("DELETE FROM member WHERE id_membre = :id");
